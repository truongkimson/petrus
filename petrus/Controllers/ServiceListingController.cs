...
namespace petrus.Controllers
{
    public class ServiceListingController : Controller
    {
        ...
        public ServiceListingController(petrusDb context, IWebHostEnvironment hostEnvironment)
        {
            dbContext = context;
        ...
        {
            var listings = dbContext.ServiceListings.ToList();
        ...
        public async Task<IActionResult> New(ServiceListingViewModel model)
        {
            if (ModelState.IsValid)
            {
                string[] uniqueFileNames = UploadedFile(model);
                ServiceListing myListing = new ServiceListing
                {
                ...
                dbContext.ServiceListings.Add(myListing);
        ...
        private string[] UploadedFile(ServiceListingViewModel model)
        {
        ...
        public IActionResult Remove(string id)
        {
            if (id != null)
            {
                ServiceListing listing = dbContext.ServiceListings.FirstOrDefault(x => x.ServiceListingID == id);
                if (listing != null)
                {
                    dbContext.ServiceListings.Remove(listing);
